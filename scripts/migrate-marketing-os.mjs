...
async function main() {
  ...
  const sql = readFileSync(join(process.cwd(), "supabase/marketing-os-migration.sql"), "utf8");
  console.log("Ejecutando supabase/marketing-os-migration.sql...");
  await client.query(sql);

  const verify = await client.query(`
    SELECT table_name FROM information_schema.tables
    WHERE table_schema = 'public'
    AND table_name IN ('monthly_marketing_plans', 'instagram_funnels')
    ORDER BY table_name
  `);
  ...
  if (verify.rows.length !== 2) {
    console.error("No se pudieron verificar las tablas de Marketing OS.");
    process.exit(1);
  }
